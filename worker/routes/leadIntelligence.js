...
// 2GIS is an official, documented REST API (catalog.api.2gis.com) — unlike Kaspi this needs no
// browser automation and no collector: the Worker calls it directly and returns results
// synchronously, the same shape the Apify-backed Google Maps search already returns to the
// frontend. Response field names (result.items[].contact_groups[].contacts[]) are based on
// 2GIS's public docs, not verified against a live call from this codebase's dev environment
// (docs.2gis.com and catalog.api.2gis.com are both unreachable there) — parsing below is
// deliberately defensive (tries a couple of likely shapes) rather than assuming one exact
// schema; adjust the mapping in discover2Gis() if a real response looks different.
async function discover2Gis(request, env) {
  ...
  url.searchParams.set('fields', 'items.contact_groups,items.point,items.address_name,items.reviews');
